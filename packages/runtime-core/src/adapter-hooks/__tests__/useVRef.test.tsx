...
describe('useVRef / useShallowVRef Test Suites', () => {
  // 1. 验证 useVRef 的基础类型响应
  it('useVRef: reacts to primitive value changes', async () => {
    let renderCount = 0;
  ...
  // 2. 验证 useVRef 的深层响应 (Deep)
  it('useVRef: reacts to deep object mutations', async () => {
    let renderCount = 0;
    const { result } = renderHook(() => {
      renderCount++;
      const state = useVRef({ a: { b: 1 } });
  ...
  // 3. 验证 useShallowVRef 的浅层特性
  it('useShallowVRef: should be isolated from deep mutations (DOM-based)', async () => {
    const holder: { current: any | null } = { current: null };

    function Comp() {
      const state = useShallowVRef({ a: 1 });
  ...
  it('should return the same ref if the value is already a RefState', () => {
    const { result: r1 } = renderHook(() => useVRef(1));
    const { result: r2 } = renderHook(() => useVRef(r1.current));

    expect(r1.current).toBe(r2.current);
  });

  // 5. 验证复杂结构 (Map) 在 RefState 中的表现
  it('handles Map in useVRef correctly', async () => {
    const { result } = renderHook(() => useVRef(new Map([['a', 1]])));
