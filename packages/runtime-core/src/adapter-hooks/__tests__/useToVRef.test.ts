...
import { useToVRef } from '../state/useToVRef';

describe('useToVRef Advanced Test Suites', () => {
  // 1. 测试属性引用模式: useToVRef(state, 'key')
  ...
  it('handles default values for undefined properties', async () => {
    const { result } = renderHook(() => {
      ...
      const countRef = useToVRef(state, 'count', 10);
      return { state, countRef };
    });
  ...
  // 2. 测试 Getter 模式: useToVRef(() => state.count * 2)
  ...
  // 3. 测试普通值模式: useToVRef(1)
  // 应该退化为 useRefState(1)
  it('creates a standard ref for primitive values', async () => {
    const { result } = renderHook(() => useToVRef(1));
