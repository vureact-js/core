...
describe('vBindStyle', () => {
  ...
  test('should override properties based on order (last wins)', () => {
    ...
    // 数组内部覆盖
    expect(vBindStyle([base, override])).toEqual({ color: 'blue', width: '10px' });
  });
  ...
  test('should merge multiple arguments (target + ...merges)', () => {
    ...
    // 模拟 vBindStyle(t1, t2, t3)
    const result = vBindStyle(t1, t2, t3);
    expect(result).toEqual({
      color: 'red',
      width: '20px',
      zIndex: 10,
