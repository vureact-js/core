...
import { generate, transform } from '@src/core';
import { readFileSync, writeFileSync } from 'fs';
import path from 'path';

function testVueTypeAsAny() {
  ...
  console.time('testVueTypeAsAny transform duration');
  console.log();

  const ast = parseSFC(content, ctx.data);
  const result = transform(ast, ctx.data);

  console.timeEnd('testVueTypeAsAny transform duration');
  console.log();
